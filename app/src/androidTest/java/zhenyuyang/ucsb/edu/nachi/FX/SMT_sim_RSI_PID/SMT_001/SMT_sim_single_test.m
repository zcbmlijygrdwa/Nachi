function result = SMT_sim_single_test()
...
addpath('../fx_util')
addpath('C:/Users/Zhenyu/Desktop/sandboxxxxx/trueFX_data_05_30')
addpath('E:/fx_EUR_USD_tick')
addpath('../../matlabplugins')
addpath('../trueFX_data')
addpath('../../../fx_EUR_USD_tick')

plotPeriodMain = 200000;
...
RSI1_state(2) = double(int32(RSI0_state(2)*1.618));; %RSI1_state.period = 200;

RSI2_state(1) = 0; %RSI1_state.initialized = false;
RSI2_state(2) = double(int32(RSI1_state(2)*1.618));; %RSI1_state.period = 200;

RSI3_state(1) = 0; %RSI1_state.initialized = false;
RSI3_state(2) = double(int32(RSI2_state(2)*1.618));; %RSI1_state.period = 200;

RSI4_state(1) = 0; %RSI1_state.initialized = false;
RSI4_state(2) = double(int32(RSI3_state(2)*1.618));; %RSI1_state.period = 200;
...
if isSim
    ...
    try
        ...
        warning('Problem in GetAccounts');
        return
    end
...
for yearIdx = 2010:2018
    for monthIdx =1:12
        ...
        while dataCount<length(sellRaw)   %test
            
            frameCount = frameCount+1;
            dataCount = dataCount+1;
            
            tic;
            %profitRate = ((((1+((((account.balance)-10000)/(days*86400+frameCount))/10000)*3600)^24)^5)^4-1);
            
            if (mod(frameCount,432000)==0)
                
                %update RSI parameter everyweek
                PID_state = pidController(PID_state,RSIHitsPerWeek);
                [RSI0_state(2),RSIHitsPerWeek]
                newP = RSI0_state(2) - PID_state.result;
                newP = double(int32(newP))
                
                if(ifPlot)
                    if(size(PeriodHist,1)>=maxPriod)
                ...
                %change period
                MA_state.period = newP;
                
                
                RSI0_state(2) = newP;
                RSI1_state(2) = double(int32(RSI0_state(2)*1.618));
                RSI2_state(2) = double(int32(RSI1_state(2)*1.618));
                RSI3_state(2) = double(int32(RSI2_state(2)*1.618));
                RSI4_state(2) = double(int32(RSI3_state(2)*1.618));
                
                
                if(ifPlot)
                    if(size(RSIHitsPerWeekHist,1)>=maxPriod)
                        %balanceHistory = balanceHistory(2:end);
            ...
            if(isSim)
                ...
                if (hours>=0 && hours<=9)||(hours>=22&&hours<=24)
                    expScale = expScaleNight;
                else
                    expScale = expScaleDay;
                end
            else
                time = clock;
                if (time(4)>=0 && time(4)<=9)||(time(4)>=23&&time(4)<=24)
                    expScale = expScaleDay;%expScaleNight;
                else
                    expScale = expScaleDay;
                end
            end
            ...
            if isSim
                %simPrice = simPrice+(rand-0.5)/3/1000;
                %         simPrice = simPrice+normrnd(0,0.00002);
                %         price.closeoutBid = num2str(simPrice,8);
                %         spreads = 1.7;
                %         price.closeoutAsk = num2str(simPrice+spreads/10000,8);
                price.closeoutBid = (sellRaw(dataCount));
                price.closeoutAsk = (buyRaw(dataCount));
                newPrice = (price.closeoutBid+price.closeoutAsk)/2;
            else
                try
                    price = GetPrices('EUR_USD');
                catch
            ...
            %             if(size(dataS,1)>=maxPriod)
            %                 dataS = dataS(2:end);
            %             end
            %             dataS = [dataS;(price.closeoutBid)];
            %
            %
            %             if(size(dataB,1)>=maxPriod)
            %                 dataB = dataB(2:end);
            %             end
            %             dataB = [dataB;(price.closeoutAsk)];
            
            if(ifPlot)
                if(size(dataN,1)>=maxPriod)
                    %dataN = dataN(2:end);
            ...
            %record price
            %     fileID = fopen('C:\Users\Zhenyu\Desktop\fx_debug\prices.txt','a');
            %     fmt = '%6f\r\n';
            %     fprintf(fileID,fmt,newPrice);
            %     fclose(fileID);
            ...
            if(buyHolds~=0||sellHolds~=0)
                ...
                if(ifPlot)
                    if(size(balanceHistory,2)>=maxPriod)
                        %balanceHistory = balanceHistory(2:end);
                ...
                %                 instantValidate;
                %                 instantValidateResult = false; %test
                %                 if(instantValidateResult)
                %                     BenefitRatioHistory = -1;
                %                     disp('instantValidate failed');
                %                     calculateScore;
                %                     return;
                %                 end
            ...
            if(ifPlot)
                if(size(filteredData0Diff,1)>=maxPriod)
                    ...
                    filteredData0Diff = [filteredData0Diff; RSI0_state(12)];
                    filteredData1Diff = [filteredData1Diff; RSI1_state(12)];
                    filteredData2Diff = [filteredData2Diff; RSI2_state(12)];
                    filteredData3Diff = [filteredData3Diff; RSI3_state(12)];
                    filteredData4Diff = [filteredData4Diff; RSI4_state(12)];
                end
            end
            ...
            %                 IP_idx_current =  size(filteredDataDiff,1);
            %                 IP_idx = [IP_idx IP_idx_current];
            %             end
            
            
            if(RSI0_state(11)>50+RSI_threshold*50||RSI0_state(11)<50-RSI_threshold*50)
                RSIHitsPerWeek = RSIHitsPerWeek +1;
            end
            
            
            if(length(simUnits)==0&&((RSI0_state(11)>50+RSI_threshold*50&&RSI1_state(11)>50+RSI_threshold*50&&RSI2_state(11)>50+RSI_threshold*50&&RSI3_state(11)>50+RSI_threshold*50&&RSI4_state(11)>50+RSI_threshold*50)||...
            ...
            if(isRSIHit...
                    ...
                    ...%&&RSI0_state.result<0.5...
                    )
                lastHitFrame = lastHitFrame
                hitSince = frameCount - lastHitFrame
                IP_idx_current =  size(filteredData0Diff,1);
                IP_idx = [IP_idx IP_idx_current];
                isRSIHit = false;
                %plotPeriod = 30;
                %beep;
...
%             end
            ...
            %buy trading
            if(buyHolds==0&&sellHolds==0&&(length(IP_idx)~=0)&&IP_idx(end)==length(data)&&RSI0_state(11)<50)
                buyCost = newPrice;
            ...
            %sell trading
            if(buyHolds==0&&sellHolds==0&&(length(IP_idx)~=0)&&IP_idx(end)==length(data)&&RSI0_state(11)>50)%&&excitingRate<excitingRateThreshold)
            ...
            if(mod(frameCount,plotPeriod)==0)
